...
- GitHub MCP connector write operations return 403 (connector is read-only)
- Workaround: use PAT + GitHub REST API via bash_tool
- PAT stored at `~/.thefretshed-mcp-env` — never commit this file
- Example: `source ~/.thefretshed-mcp-env && curl -X POST ... -H "Authorization: Bearer $GITHUB_TOKEN"`
...
**Login:** `login.html` — hardcoded username/password per user. On success, sets
`userId` in `sessionStorage`. `index.html` redirects to login if no session found.
...
**Gear images:** Hardcoded in `index.html` via `images/` folder — no localStorage dependency.

---

## Known Issues (Active Bugs)

Tracked in GitHub Issues under the **Bugs** column on the project board. None currently open.
...
progress.js API layer added, login.html live. PRs #53 merged. GitHub write ops now handled
via PAT + REST API (MCP connector is read-only). Next: #49 Unified Session View.*
